...
?>
    ...
    <div id="itemList" style="overflow:auto;height:620px;" class="bg-body-secondary p-1">
        <div class="card mb-3 shadow-sm rounded-1 border-0 item-card">
            ...
            <div class="card mb-3 shadow-sm rounded-1 border-0 item-card" data-name="<?= strtolower($key['item_name']); ?>">
                <div class="card-body p-2">
                    <div class="row">
                            ...
                            <?php if ($key['item_id'] != 2) { ?>
                                ...
                                ");
                                    ...
                                    foreach ($item_types as $type):
                                        $isFirst = ($i === 0);
                                    ?>
                                        <a href="javascript:void(0)" class="badge rounded-1 bg-success fs-10 p-1 text-decoration-none <?= $isFirst ? 'bg-success text-white active' : 'bg-dark-subtle text-black' ?>"
                        ...
                        <div class="col-lg-9 col-12">
                            <h6 class="mb-0"><?= $key['item_name']; ?></h6>
                            <ul class="nav nav-pills mb-3 mt-2 product_search_list" id="pills-tab" role="tablist">
                                <?php
                                $item_types = $obj->executequery("
                                    SELECT item_type_master_id,item_type_master_name
                                    FROM item_type_master
                                ...
                                ?>
                                    <li class="nav-item" role="presentation">
                                        <a href="javascript:void(0)" class="badge nav-link <?= $isFirst ? "active" : "" ?>" id="pills-home-tab" data-bs-toggle="pill" data-bs-target="#pills-home_<?= $type['item_type_master_id']; ?>" role="tab" aria-controls="pills-home" aria-selected="true" data-item="<?= $key['item_id']; ?>"
                                            data-type="<?= $type['item_type_master_id']; ?>">
                                            <?= $type['item_type_master_name']; ?>
                                        </a>
                                    </li>
                            ...
                            <div class="tab-content item-service" id="pills-tabContent">
                                <?php $item_services = $obj->executequery("SELECT 
        A.add_service_id,
        A.rate,
        B.item_sname,
        B.is_pressing,
...
");
                                foreach ($item_services as $srv): ?>
                                    <div class="tab-pane fade show active" id="pills-home_<?= $type['item_type_master_id']; ?>" role="tabpanel" aria-labelledby="pills-home-tab" tabindex="0"> <?= $srv['item_sname']; ?> ₹ <?= $srv['rate']; ?> / <?= ($item_in == 1) ? 'PR' : 'PC'; ?></div>
                                <?php endforeach; ?>
                            </div>
                        </div>
                        <div class="col-lg-3 col-3 d-flex justify-content-center align-items-center text-center">
